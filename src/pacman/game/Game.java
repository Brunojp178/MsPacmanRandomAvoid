...
public final class Game {
	...
	public MOVE getNextMoveAwayFromTarget(int fromNodeIndex, int toNodeIndex, DM distanceMeasure) {
        ...
        /**
         * Function that move away from a target, but choosing a random direction
         * 
         * @param fromNodeIndex
         * @param toNodeIndex
         * @param distanceMeasure
         * @return 
         */
        public MOVE getNextMoveAwayRandom(int fromNodeIndex, int toNodeIndex, DM distanceMeasure){
            MOVE move = null;
            Random rand = new Random();
            int RandomMaxValue = 101;
            
            double minDistance = getDistance(fromNodeIndex, toNodeIndex, distanceMeasure);
            
            // Run the array of possible moves of pacman
            for (Entry<MOVE, Integer> entry : currentMaze.graph[fromNodeIndex].neighbourhood.entrySet()) {
                // Calculate the distance from possible path to ghost    
                double distance = getDistance(entry.getValue(), toNodeIndex, distanceMeasure);
                    
                    // If the distance from possible path is shorter then the distance of the pacman to the ghost, ignore it
                    if (!(distance < minDistance) && rand.nextInt(RandomMaxValue) < 20) {
                        // if it enters the random range simple choose this path
                        move = entry.getKey();
                    }
            }
            return move;
